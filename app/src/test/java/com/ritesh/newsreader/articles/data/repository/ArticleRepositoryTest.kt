...
class ArticleRepositoryTest {
    ...
    @Test
    fun getSavedArticles_shouldReturnSuccess() {
        runTest {
            val response = getSavedArticlesMockedSuccessResponse()
            Mockito.doReturn(flowOf(response)).`when`(database).getSavedArticles()

            database.getSavedArticles().test {
    ...
    private fun getCountriesMockedSuccessResponse(): List<Country> {
        return listOf(
            Country("India", "in"),
            Country("Israel", "il"),
        )
